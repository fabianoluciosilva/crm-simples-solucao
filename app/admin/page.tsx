...
return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      ...
      <Sidebar menuMobileAberto={menuMobileAberto} setMenuMobileAberto={setMenuMobileAberto} tema={tema} alternarTema={() => setTema(tema === 'dark' ? 'light' : 'dark')} aba={aba} mudarAba={(a: any) => setAba(a)} isComercial={isComercial} isAdmin={isAdmin} tarefasUrgentesCount={0} router={router} abrirBuscaGlobal={() => {}} perfilAtivo={perfilAtivo} handleLogout={() => supabase.auth.signOut()} />
      ...
      {clienteDetalhe && (
        <ModalFichaCliente 
          clienteDetalhe={clienteDetalhe} setClienteDetalhe={setClienteDetalhe} isComercial={isComercial} isAdmin={isAdmin} abrirNovaTarefa={() => setModalTarefa(true)} formInteracao={{tipo:'Nota', descricao:''}} setFormInteracao={()=>{}} salvarInteracao={async(e:any)=>{e.preventDefault(); carregarTudo();}} 
        />
      )}
      ...
      {modalEnvioProposta.ativo && (
        <ModalEnvio modalEnvioProposta={modalEnvioProposta} setModalEnvioProposta={setModalEnvioProposta} formEnvioMensagem={{templateId:'', texto:'', assunto:''}} setFormEnvioMensagem={()=>{}} confirmarEnvioMensagem={async()=>{setModalEnvioProposta({...modalEnvioProposta, ativo:false}); showToast("Simulação enviada!");}} templates={templates} />
      )}

      {modalTarefa && <ModalTarefa isOpen={true} onClose={() => setModalTarefa(false)} formTarefa={{}} setFormTarefa={()=>{}} salvarTarefa={async()=>{}} />}

      {toast && (
        <div style={{ position: "fixed", bottom: 20, right: 20, background: toast.tipo === 'erro' ? '#ef4444' : '#22c55e', color: '#fff', padding: '12px 24px', borderRadius: 10, zIndex: 10000 }}>
          {toast.msg}
        </div>
      )}
    </div>
