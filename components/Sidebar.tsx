...
interface SidebarProps {
  ...
  router: any;
  abrirBuscaGlobal: () => void;
  perfilAtivo: any;
  handleLogout: () => void;
}

export const Sidebar = ({
  menuMobileAberto, setMenuMobileAberto, tema, alternarTema, aba, mudarAba,
  isComercial, isAdmin, tarefasUrgentesCount, router, abrirBuscaGlobal, perfilAtivo, handleLogout
}: SidebarProps) => {
  return (
    <aside className={`sidebar ${menuMobileAberto ? 'open' : ''}`}>
      ...
      <div style={{ padding: "16px 20px", borderTop: "1px solid var(--border-light)" }}>
        ...
        <div style={{ fontSize: "10px", color: "var(--text-tertiary)", marginTop: 10, textAlign: "center" }}>v4.5 - Layout Extraído</div>
      </div>
    </aside>
  );
};
