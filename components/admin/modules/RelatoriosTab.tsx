...
export function RelatoriosTab({
  clientesAgrupados, contratos, mrrAtivo, ticketMedio, dadosPipelineMensal, fmt
}: any) {
  return (
    <>
      <div className="grid-metrics" style={{ marginBottom: 24 }}>
        <MetricCard label="CLIENTES ATIVOS" value={clientesAgrupados.filter((c:any) => c.tipo === 'Cliente' && c.ativo !== false).length} icon="👥" />
        <MetricCard label="LEADS NA BASE" value={clientesAgrupados.filter((c:any) => c.tipo === 'Lead').length} color="#f59e0b" icon="🎯" />
        <MetricCard label="CONTRATOS ATIVOS" value={contratos.filter((c:any) => c.status === 'Ativo').length} color="#22c55e" icon="📄" />
        <MetricCard label="CHURN (CANCELADOS)" value={contratos.filter((c:any) => c.status === 'Cancelado').length} color="#f87171" icon="📉" />
