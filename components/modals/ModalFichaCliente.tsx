  ...
  const timeline = useMemo(() => {
    ...
    clienteDetalhe.propostas?.forEach((p: any) => {
      eventos.push({
        ...
        desc: `Valor: ${fmt(p.valor)} - Status: ${p.status}`,
        tipo: 'proposta',
        icon: '📄'
      });
    });
    ...
    <div className="modal-overlay" onClick={() => setClienteDetalhe(null)}>
      <div className="modal-content" style={{ maxWidth: "800px", width: "95%" }} onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
          <div>
            <h2 style={{ fontSize: 24 }}>{clienteDetalhe.nome}</h2>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 30 }}>
          ...
          <div>
            <h3 style={{ fontSize: 16, marginBottom: 15 }}>Nova Interação</h3>
            <form onSubmit={salvarInteracao}>
              <select 
                value={formInteracao.tipo} 
                onChange={e => setFormInteracao({...formInteracao, tipo: e.target.value})}
                style={{ width: "100%", padding: 10, marginBottom: 10, background: "var(--bg-main)", border: "1px solid var(--border-light)", color: "var(--text-primary)", borderRadius: 8 }}
              >
              ...
              <textarea 
                ...
                style={{ width: "100%", height: 100, padding: 10, marginBottom: 10, background: "var(--bg-main)", border: "1px solid var(--border-light)", color: "var(--text-primary)", borderRadius: 8 }}
                required
              />
          ...
          <div style={{ maxHeight: "400px", overflowY: "auto", paddingRight: 10 }}>
            <h3 style={{ fontSize: 16, marginBottom: 15 }}>Linha do Tempo</h3>
            <div style={{ position: "relative", borderLeft: "2px solid var(--border-light)", marginLeft: 10, paddingLeft: 20 }}>
              {timeline.length === 0 && <p style={{ color: "var(--text-tertiary)", fontSize: 13 }}>Nenhum histórico registado.</p>}
              
              {timeline.map((item, idx) => (
                <div key={idx} style={{ marginBottom: 20, position: "relative" }}>
                  <div style={{ position: "absolute", left: "-31px", top: 0, background: "var(--bg-sidebar)", padding: "2px" }}>
                    {item.icon}
                  </div>
